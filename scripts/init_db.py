...
async def create_sample_data(db):
    ...
    for analysis in sample_analyses:
        ...
        VALUES (:document_id, :analysis_type, :result, :confidence, datetime('now'))
        """
        await db.execute(query, analysis)
...
async def verify_database(db):
    ...
    result = await db.fetch_all(tables_query)
...
async def main():
    ...
    try:
        ...
        if args.verify_only:
            ...
            success = await verify_database(db)
            if success:
        ...
        print("2. Start the application: make serve")
